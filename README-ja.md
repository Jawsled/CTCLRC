...
CTCLRC は、以下の 2 つのファイルから **LRC 歌詞タイミングファイル** を生成する Windows & Linuxデスクトップアプリです。
...
1. [リリース](https://github.com/Kdroidwin/CTCLRC/releases)から`CTCLRC.exe`をダウンロード
2. `CTCLRC.exe` を起動します。
3. 音声ファイルを選択します。
4. 歌詞の `.txt` ファイルを選択します。
5. 日本語の楽曲の場合は、言語を **Japanese (jpn)** のままにします。
6. **Generate LRC** をクリックします。
